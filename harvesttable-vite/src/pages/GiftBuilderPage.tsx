...
const GiftBuilderPage: React.FC = () => {
  ...
  const available = PRODUCTS.filter(p => !p.category.includes('gift') && (!filterCat || p.category === filterCat));
        ...
        <div style={{
          ...
          <div style={{ position: 'relative', zIndex: 2, maxWidth: 1200, margin: '0 auto', padding: '72px 32px', width: '100%' }}>
            ...
            <div style={{
              ...
              height: 2,
              width: 60,
              background: 'linear-gradient(90deg, #7a4a28, #d4a870, #7a4a28)',
          ...
          <div style={{ ...f(0.2), display: 'flex', alignItems: 'center', gap: 8, marginBottom: 40 }}>
            {stepLabels.map((s, i) => (
              <React.Fragment key={s}>
                <div style={{
                  display: 'flex', alignItems: 'center', gap: 8, cursor: i + 1 < step ? 'pointer' : 'default',
            ...
            <div style={{ animation: 'stepIn 0.5s cubic-bezier(0.22,1,0.36,1) both' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, maxWidth: 640, margin: '0 auto 32px' }}>
                {GIFT_SIZES.map((size, i) => (
                  <button key={size.labelKey} onClick={() => setSelectedSize(i)} style={{
                    ...
                    <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 18, fontWeight: 600, marginBottom: 8, color: selectedSize === i ? C.accent : C.heading }}>{t(size.labelKey)}</h3>
                    <p style={{ fontSize: 12, lineHeight: 1.6, color: C.muted, marginBottom: 12 }}>{t(size.descKey)}</p>
                    <span style={{ fontSize: 12, fontWeight: 600, color: C.price }}>{t('gift.upTo')} {size.items} {t('gift.items')} · +${size.price}</span>
                  </button>
                ))}
              </div>
              <div style={{ textAlign: 'center' }}>
                <button onClick={() => { setSelectedItems(p => p.slice(0, maxItems)); setStep(2); }} style={{
                  ...
                  letterSpacing: '0.14em', textTransform: 'uppercase', color: '#fff', backgroundColor: C.accent, border: 'none',
          ...
          {step === 2 && (
            <div style={{ animation: 'stepIn 0.5s cubic-bezier(0.22,1,0.36,1) both' }}>
              <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 20 }}>
                <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 28, fontWeight: 600, color: C.heading, margin: 0 }}>{t('gift.selectItems')}</h2>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  {selectedItems.length === maxItems && (
                    <span style={{
                      ...
                      backgroundColor: C.greenBg, color: C.green, border: '1px solid rgba(58,96,40,0.2)',
                    }}>{t('gift.boxFull')}</span>
                  )}
                </div>
              </div>
              ...
              <div style={{ marginBottom: 24 }}>
                <div style={{ height: 4, borderRadius: 4, overflow: 'hidden', backgroundColor: C.border }}>
                  <div style={{
                    ...
                    width: `${pct}%`, backgroundColor: pct === 100 ? C.green : C.accent,
                ...
                {available.map((product, i) => {
                  ...
                  return (
                    <button key={product.id} onClick={() => toggleItem(product)} disabled={isDisabled} style={{
                      borderRadius: 12, overflow: 'hidden', textAlign: 'left', cursor: isDisabled ? 'not-allowed' : 'pointer',
                      ...
                      <div style={{ position: 'relative' }}>
                        <ProductImage type={product.imageType} name={product.name} className="h-24 w-full" />
                        {isSelected && (
                          <div style={{
                            position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center',
                            backgroundColor: 'rgba(122,74,40,0.2)',
                          }}>
                            <div style={{
                              width: 32, height: 32, borderRadius: '50%', backgroundColor: C.accent,
                              display: 'flex', alignItems: 'center', justifyContent: 'center',
                              boxShadow: '0 2px 8px rgba(122,74,40,0.4)',
                            }}>
                      ...
                      <div style={{ padding: '10px', backgroundColor: C.surface }}>
                        <p style={{ fontSize: 12, fontWeight: 600, color: C.heading, margin: '0 0 2px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{product.name}</p>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                ...
                <button onClick={() => setStep(3)} disabled={selectedItems.length === 0} style={{
                  ...
                  letterSpacing: '0.10em', textTransform: 'uppercase', color: '#fff', backgroundColor: C.accent, border: 'none',
          ...
          {step === 3 && (
            <div style={{ maxWidth: 420, margin: '0 auto', animation: 'stepIn 0.5s cubic-bezier(0.22,1,0.36,1) both' }}>
              <div style={{ textAlign: 'center', marginBottom: 32 }}>
                ...
                <p style={{ fontSize: 13, color: C.muted, margin: 0 }}>{selectedItems.length} {t('gift.items')} {t('gift.selected')}</p>
              </div>

              <div style={{
                borderRadius: 16, padding: 20, marginBottom: 24,
                backgroundColor: C.surface, border: `1px solid ${C.border}`,
                boxShadow: '0 2px 16px rgba(122,74,40,0.08)',
              }}>
                <h3 style={{ fontSize: 10, letterSpacing: '0.24em', textTransform: 'uppercase', fontWeight: 700, color: C.label, marginBottom: 16 }}>{t('gift.contents')}</h3>
                  ...
                  {selectedItems.map((item, idx) => (
                    <div key={item.id} style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <ProductImage type={item.imageType} name={item.name} className="w-8 h-8 rounded-lg flex-shrink-0" />
                        <span style={{ fontSize: 13, color: C.body }}>{item.name}</span>
                      </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <button onClick={handleAddToCart} style={{
                  ...
                  ...(added
                    ...
                    : { backgroundColor: C.accent, color: '#fff', boxShadow: '0 4px 20px rgba(122,74,40,0.28)' }
                  ),
                }}
                ...
                <button onClick={() => setStep(2)} style={{
                  width: '100%', padding: '12px 0', fontSize: 11, letterSpacing: '0.10em', textTransform: 'uppercase', borderRadius: 2,
