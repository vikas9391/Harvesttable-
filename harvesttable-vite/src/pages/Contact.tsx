...
const Contact: React.FC = () => {
  ...
  const inputStyle: CSSProperties = {
    width:"100%", padding:"13px 16px", borderRadius:10, border:"1px solid #ede5d8",
    fontSize:15, fontFamily:"'Jost',sans-serif", color:"#2a1a0e",
    background:"#faf7f2", outline:"none", transition:"border-color .2s,box-shadow .2s",
        ...
        <div style={{ position:"relative",minHeight:320,display:"flex",alignItems:"center",overflow:"hidden",background:"linear-gradient(135deg,#0e1a2a 0%,#102040 60%,#182a50 100%)" }}>
          ...
          <div style={{ position:"relative",zIndex:2,maxWidth:960,margin:"0 auto",padding:"80px 32px",width:"100%" }}>
            <div style={{ ...f(0.05),display:"flex",alignItems:"center",gap:12,marginBottom:14 }}>
              ...
              <p style={{ fontSize:10,letterSpacing:"0.42em",textTransform:"uppercase",fontWeight:700,color:"#70a8d4",margin:0 }}>{t('contact.hero.eyebrow')}</p>
            </div>
            <h1 style={{ ...f(0.12),fontFamily:"'Cormorant Garamond',serif",fontSize:"clamp(34px,6vw,62px)",fontWeight:600,color:"#faf2e8",lineHeight:1.08,margin:"0 0 16px" }}>{t('contact.hero.title')}</h1>
            ...
            <p style={{ ...f(0.24),fontSize:16,fontWeight:300,lineHeight:1.9,color:"rgba(250,242,232,0.75)",maxWidth:460,margin:0 }}>{t('contact.hero.subtitle')}</p>
          </div>
        </div>
        ...
        <div style={{ maxWidth:960,margin:"0 auto",padding:"72px 24px" }}>
          ...
          <div ref={r1} style={{ marginBottom:72 }}>
            <Slide delay={0} active={v1}>
              <p style={{ fontSize:11,letterSpacing:"0.35em",textTransform:"uppercase",fontWeight:700,color:"#9a6840",marginBottom:10 }}>{t('contact.section1.eyebrow')}</p>
              <h2 style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:"clamp(26px,4vw,40px)",fontWeight:600,color:"#2a1a0e",marginBottom:32,lineHeight:1.1 }}>{t('contact.section1.title')}</h2>
            </Slide>
          ...
          <div ref={r2} style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:32,alignItems:"start" }}>
            ...
            <Slide delay={0} active={v2} dir="left">
              <div style={{ background:"#fff",border:"1px solid #ede5d8",borderRadius:20,padding:"36px 32px" }}>
                <p style={{ fontSize:11,letterSpacing:"0.35em",textTransform:"uppercase",fontWeight:700,color:"#9a6840",marginBottom:10 }}>{t('contact.form.eyebrow')}</p>
                <h2 style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:"clamp(24px,3.5vw,34px)",fontWeight:600,color:"#2a1a0e",marginBottom:6,lineHeight:1.1 }}>{t('contact.form.title')}</h2>
                ...
                {sent ? (
                  <div style={{ textAlign:"center",padding:"40px 20px" }}>
                    ...
                    <h3 style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:24,fontWeight:700,color:"#2a1a0e",marginBottom:10 }}>{t('contact.form.sent.title')}</h3>
                    <p style={{ fontSize:15,color:"#8a7060",lineHeight:1.75 }}>{t('contact.form.sent.desc')}</p>
                  </div>
                ) : (
                  <form onSubmit={handleSubmit}>
                    <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14 }}>
                      <div>
                        <label style={{ display:"block",fontSize:12,fontWeight:600,color:"#5a4030",marginBottom:6,letterSpacing:"0.04em" }}>{t('contact.form.name')}</label>
                        <input required className="contact-input" style={inputStyle} placeholder={t('contact.form.name.placeholder')} value={form.name} onChange={e=>setForm(p=>({...p,name:e.target.value}))}/>
                      </div>
                      <div>
                        <label style={{ display:"block",fontSize:12,fontWeight:600,color:"#5a4030",marginBottom:6,letterSpacing:"0.04em" }}>{t('contact.form.email')}</label>
                        <input required type="email" className="contact-input" style={inputStyle} placeholder={t('common.emailPlaceholder')} value={form.email} onChange={e=>setForm(p=>({...p,email:e.target.value}))}/>
                      </div>
                    </div>
                    <div style={{ marginBottom:14 }}>
                      <label style={{ display:"block",fontSize:12,fontWeight:600,color:"#5a4030",marginBottom:6,letterSpacing:"0.04em" }}>{t('contact.form.subject')}</label>
                      <input required className="contact-input" style={inputStyle} placeholder={t('contact.form.subject.placeholder')} value={form.subject} onChange={e=>setForm(p=>({...p,subject:e.target.value}))}/>
                    </div>
                    <div style={{ marginBottom:24 }}>
                      <label style={{ display:"block",fontSize:12,fontWeight:600,color:"#5a4030",marginBottom:6,letterSpacing:"0.04em" }}>{t('contact.form.message')}</label>
                      <textarea required className="contact-input" style={{ ...inputStyle,minHeight:130,resize:"vertical" }} placeholder={t('contact.form.message.placeholder')} value={form.message} onChange={e=>setForm(p=>({...p,message:e.target.value}))}/>
                    </div>
                    <button type="submit" disabled={sending} style={{ width:"100%",padding:"14px 24px",borderRadius:12,border:"none",cursor:sending?"not-allowed":"pointer",background:sending?"rgba(122,74,40,0.6)":"#7a4a28",color:"#fff",fontSize:15,fontWeight:600,fontFamily:"'Jost',sans-serif",letterSpacing:"0.04em",display:"flex",alignItems:"center",justifyContent:"center",gap:8,transition:"background .2s,box-shadow .2s",boxShadow:"0 4px 20px rgba(122,74,40,0.28)" }}
                      ...
                      {sending ? (
                        <><div style={{ width:16,height:16,border:"2px solid rgba(255,255,255,0.4)",borderTopColor:"#fff",borderRadius:"50%",animation:"spin .7s linear infinite" }}/> {t('contact.form.sending')}</>
                      ) : t('contact.form.send')}
                    </button>
                  </form>
                )}
            ...
            <div>
              <Slide delay={0.12} active={v2} dir="right">
                <div style={{ background:"#fff",border:"1px solid #ede5d8",borderRadius:16,padding:"28px 24px",marginBottom:16 }}>
                  <h3 style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:22,fontWeight:700,color:"#2a1a0e",marginBottom:16 }}>{t('contact.times.title')}</h3>
                  {responseTimes.map((row,i) => (
                    <div key={i} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:i<responseTimes.length-1?"1px solid #f4ede4":"none" }}>
                      ...
                      <span style={{ fontSize:13,fontWeight:600,color:"#7a4a28",background:"rgba(122,74,40,0.08)",padding:"3px 10px",borderRadius:99,border:"1px solid rgba(122,74,40,0.15)" }}>{row.time}</span>
                    </div>
                  ))}
                </div>
              </Slide>
              <Slide delay={0.2} active={v2} dir="right">
                <div style={{ background:"linear-gradient(135deg,#2a1a0e,#4a2a10)",border:"1px solid rgba(212,168,112,0.2)",borderRadius:16,padding:"28px 24px" }}>
                  ...
                  <h3 style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:22,fontWeight:700,color:"#faf2e8",marginBottom:10 }}>{t('contact.did.title')}</h3>
                  <p style={{ fontSize:14,lineHeight:1.8,color:"rgba(250,242,232,0.72)",margin:"0 0 18px",fontWeight:300 }}>{t('contact.did.desc')}</p>
                  <div style={{ display:"flex",alignItems:"center",gap:10 }}>
                    ...
                    <span style={{ fontSize:13,color:"rgba(212,168,112,0.85)",fontWeight:500 }}>{t('contact.did.location')}</span>
                  </div>
                </div>
              </Slide>
            </div>
